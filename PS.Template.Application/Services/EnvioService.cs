    ...
    public class EnvioService : IEnvioService
    {
        ...
        public ResponseRequestDto CreateEnvioPaquetes(RequestEnvioPaquetesDto envio)
        {
            ...
            if (!ValidarTipoPaquetes(paquetes))
                return new ResponseRequestDto { Codigo = 400, Mensaje = "No se puede enviar distintos tipos de paquetes en un envío" };
        ...
        {
            List<ResponseEnvioDto> envios = _query.GetEnviosQuery(unIdEnvio, unIdUsuario);
            ...
            if (envios != null)
            {
                ...
                {
                    paquetes = _query.GetPaquetes(envio.IdEnvio);
                    direccion = ObtenerDirDestino(envio.IdDireccionDestino);
                    ...
                    envioCompleto.Add(new ResponseEnvioCompleto
                    {
                        ...
                        CpLocalidad = direccion.CpLocalidad,
        ...
        public ResponseGetDireccion ObtenerDirDestino(int idDireccion)
        {
            string uri = _generateRequest.GetUri(3);
            uri += "/" + idDireccion.ToString();
            RestRequest request = new RestRequest(Method.GET);
            ResponseGetDireccion direccion = _generateRequest.ConsultarApiRest<ResponseGetDireccion>(uri, request).First();
            return direccion;
        }
    }
}